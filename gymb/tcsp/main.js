...
function getkuaishouid(){
	...
	if(kuauishouurl.indexOf('gifshow.com')<0 && kuauishouurl.indexOf('kuaishou.com')<0 && kuauishouurl.indexOf('kwai.com')<0){layer.alert('请输入正确的快手作品链接！');return false;}
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				item += '<tr><td colspan="6" style="text-align:center"><b>订单基本信息</b></td></tr><tr><td class="info">商品名称</td><td colspan="5">'+data.name+'</td></tr><tr><td class="info">订单金额</td><td colspan="5">'+data.money+'元</td></tr><tr><td class="info">购买时间</td><td colspan="5">'+data.date+'</td></tr><tr><td class="info">下单信息</td><td colspan="5">'+data.inputs+'</td></tr>';
				if(data.list){
